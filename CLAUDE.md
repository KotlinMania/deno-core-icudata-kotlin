# Claude Code Project Instructions — deno-core-icudata-kotlin

## Project Overview

This is **deno-core-icudata-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`deno_core_icudata`](https://crates.io/crates/deno_core_icudata).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/deno_core_icudata/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/deno_core_icudata>
