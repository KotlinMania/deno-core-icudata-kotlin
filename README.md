...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`deno_core_icudata`](https://crates.io/crates/deno_core_icudata).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `deno_core_icudata` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

ICU data bundle used by the Deno V8 runtime
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:deno-core-icudata-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `deno_core_icudata` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
